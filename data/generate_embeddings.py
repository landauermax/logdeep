...
def generate_embeddings_fasttext(templates: List[str], strategy: str = 'average') -> dict:
    ...
    if strategy == 'average':
        ...
        for template, k in templates:
            embeddings[str(i)] = np.mean(log_key2vec(template), axis=0).tolist()
            i += 1
    elif strategy == 'tfidf':
        ...
        for i, (template, k) in enumerate(templates):
            ...
            embeddings[str(i)] = np.mean(log_key2vec(template, single_weights), axis=0).tolist()
    else:
...
if __name__ == '__main__':
    ...
    template_df = pd.read_csv(templates) #f'./{dataset}/{dataset}.log_templates.csv')
