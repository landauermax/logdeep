...
def do_sample(source, train_ratio, sample_ratio, tw):
    ...
    labels = {}
    with open(source + '/parsed.csv') as extracted, open(source + '/' + source.split('/')[-1].split('_')[0] + '_train', 'w+') as train, open(source + '/' + source.split('/')[-1].split('_')[0] + '_test_normal', 'w+') as test_norm, open(source + '/' + source.split('/')[-1].split('_')[0] + '_test_abnormal', 'w+') as test_abnormal:
        cnt = 0
