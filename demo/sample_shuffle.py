...
def do_sample(source, train_ratio):
    sequences_extracted = []
    with open(source + '/' + source.split('/')[-1].split('_')[0] + '_train') as train, open(source + '/' + source.split('/')[-1].split('_')[0] + '_test_normal') as test_norm:
    ...
    with open(source + '/' + source.split('/')[-1].split('_')[0] + '_train', 'w+') as train, open(source + '/' + source.split('/')[-1].split('_')[0] + '_test_normal', 'w+') as test_norm:
